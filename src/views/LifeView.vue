...
import { getEarthGallery } from '../utils/gallery'
...
onMounted(() => {
  // 从工具函数获取动态匹配的画廊数据
  rawGalleryItems.value = getEarthGallery()
})
  ...
  <div class="page-container life-page">
    <div class="content-wrapper">
      ...
      <div class="card intro-card">
        ...
        <h1 class="title">{{ t('life.title') }}</h1>
        <p class="subtitle">{{ t('life.subtitle') }}</p>
        <div class="divider"></div>
        <p class="description">
          {{ t('life.description') }}
        </p>
        <button class="btn btn-life" @click="goBack">
      ...
      <div class="gallery-section">
        <div class="section-header">
          <h2 class="section-title">📷 {{ t('life.gallerySection') }}</h2>
          <!-- 滚动导航按钮 (PC端/平板端可见) -->
        ...
        <div class="gallery-scroll-container" ref="scrollContainer">
          ...
          <div v-if="galleryItems.length === 0" class="empty-gallery">
            <p>暂无图片，请在 src/img/4Yearth 目录中放置图片并配置 info.json</p>
          </div>
        </div>
      </div>
    </div>
...
.icon-wrapper {
  ...
  border-radius: 50%;
  text-align: center;
  box-shadow: 0 4px 12px rgba(251, 194, 235, 0.35);
  display: inline-flex;
  align-items: center;
...
.btn {
  grid-column: 4;
  grid-row: 1 / span 2;
  background: linear-gradient(135deg, #fbc2eb 0%, #e6a8d7 100%);
  color: #2c3e50;
  border: none;
  padding: 12px 26px;
  font-size: 15px;
  ...
  justify-content: center;
  gap: 8px;
  box-shadow: 0 5px 15px rgba(251, 194, 235, 0.3);
  white-space: nowrap;
  width: auto;
}

.btn:hover {
  background: linear-gradient(135deg, #e6a8d7 0%, #fbc2eb 100%);
  box-shadow: 0 6px 18px rgba(251, 194, 235, 0.4);
  transform: translateY(-2px);
}
...
.tag-date {
  ...
  top: 15px;
  right: 15px;
  background: rgba(251, 194, 235, 0.95);
  color: #2c3e50;
  padding: 5px 12px;
  border-radius: 20px;
  font-size: 12px;
